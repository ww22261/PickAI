...
async function generateQuestion1(userInput, excludeTags = []) {
  ...
  ? `\n【换一换排除词】：「${excludeTags.join('、')}」，本次所有标签的value必须与排除词完全不同，不能语义重复。`
  : '';
...
【第一步】从用户意图中提取"已明确表达的属性词"，这些词禁止出现在任何标签中：
例如用户说"便携水杯"，则"便携"是已知信息，不能再作为标签。

【第二步】识别核心品类，问题标题必须包含品类名称，禁止泛化问法。
✅ 好示例："保温杯你更看重哪个核心功能？"
❌ 坏示例："你更喜欢哪种整体风格？"

【第三步】生成标签，严格按以下规则：
- 总数 10-12 个
- 价格标签 3 个，必须符合该品类在拼多多的真实价格带，value格式：PRICE:MIN-MAX
- 功能/形态标签 7-9 个，必须覆盖至少4个不同维度（如材质、容量、使用场景、特殊功能）
- 同一维度最多出现 2 个标签
- 标签之间不能语义重叠`;
...
【第一步】提取双重排除词，这些词禁止出现在任何标签中：
- 来自用户原始输入的属性词
- 来自已选标签的所有value词：${selectedTags}

【第二步】判断问题一已覆盖的维度（通常是核心功能+预算），本次必须切换到全新维度。
优先从以下选1个最适合该品类的主维度展开：
· 使用场景（上班/户外/宿舍/健身房）
· 受众特征（送礼/自用/学生党/情侣）
· 外观偏好（颜色系/材质质感/风格）
· 附加痛点（清洗方便/防摔/防漏/单手开盖）

【第三步】生成问题标题，必须融入已选标签的1-2个核心词，体现记忆感。
✅ 好示例："你优先考虑保温，那平常是通勤用，还是放在办公室用？"
❌ 坏示例："你更偏好什么风格？"
